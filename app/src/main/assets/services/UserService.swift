...
class UserService: NSObject {
    ...
    override init() {
        super.init()
        ref = FIRDatabase.database().reference(withPath: UserService.path)
        ref.keepSynced(true)
    }
    ...
    public func configureDatabase(finish: @escaping () -> Void) {
        ...
        ref.observe(.value, with: {(snapshot) -> Void in
            debugPrint("FINISH GET USER", self.users.count, "users")
            finish()
        })
    }
    
    // MARK: - Add User
    public func addUser(_ user: FIRUser?, loginProvider s: String){
