...
class FUser: FObject {
    ...
    // init
    convenience init(snapshot snap: FIRDataSnapshot){
        self.init(path: UserService.path, snapshot: snap)
    }
    
    class func userWith(firUser u: FIRUser, loginProvider lp: String) -> FUser {
        ...
        user.dictionary = [FUser.name : (u.displayName) ?? "",
                           ...
                           FUser.FCMToken: FIRInstanceID.instanceID().token() ?? ""] as [String : Any]
        return user
    }
